...
def overlay_on_original(original_path, mask, overlay_color=(255, 255, 0), alpha=0.6):
...
def process_cork_folder(
    ...
    for i, f in enumerate(img_files):
        mask, arr = mask_pink_regions(os.path.join(cork_folder_path, f))
    ...
    orig_mask_files = [os.path.join(cork_folder_path, f) for f in fnames]
    contour_files = [
