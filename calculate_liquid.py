...
def mask_quality_score(mask, orig_arr):
    ...
    ar_score = 1.0 - abs(ar - 0.5)
    ar_score = max(ar_score, 0.0)
...
def overlay_on_original(original_path, mask, overlay_color=(255, 128, 0), alpha=0.5):
...
def process_liquid_folder(
    ...
    img_files = sorted(
        [
            f
            for f in os.listdir(liquid_folder_path)
            if f.lower().endswith((".png", ".jpg", ".jpeg"))
        ]
    )
    img_files = img_files[:2]
    for i, f in enumerate(img_files):
        mask, arr = mask_pink_regions(os.path.join(liquid_folder_path, f))
    ...
    if len(masks) == 0:
        raise RuntimeError(
            "No valid liquid mask images found (all below area threshold)."
        )
    ...
    for idx in valid_mask_indices:
        ...
        out_fname = f"{Path(orig_img_path).stem}_combinedliquid.png"
        out_img.save(os.path.join(out_dir, out_fname))
        combined_outputs.append((out_fname, out_img))

    orig_mask_files = [os.path.join(liquid_folder_path, f) for f in fnames]
    contour_files = [
